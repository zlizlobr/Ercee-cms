...
namespace Modules\Funnel\Filament\Resources;
...
use Modules\Funnel\Filament\Resources\FunnelResource\Pages;
use Modules\Funnel\Filament\Resources\FunnelResource\RelationManagers;
...
class FunnelResource extends Resource
{
    ...
    protected static function getStepLabel(array $state): string
    {
        ...
        return match ($type) {
            FunnelStep::TYPE_DELAY => 'Delay: '.($state['config']['seconds'] ?? 0).'s',
            FunnelStep::TYPE_EMAIL => 'Email: '.($state['config']['subject'] ?? 'No subject'),
            FunnelStep::TYPE_WEBHOOK => 'Webhook: '.($state['config']['url'] ?? 'No URL'),
            FunnelStep::TYPE_TAG => 'Tag: '.($state['config']['tag'] ?? 'No tag'),
            default => $typeName,
        };
    }
