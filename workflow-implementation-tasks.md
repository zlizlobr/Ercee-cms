...
<<<<<<< HEAD
- [x] Add GitHub branch protection automation script in `scripts/workflow/apply-branch-protection.sh` with rollout runbook in `docs/workflow/rollout/branch-protection-automation.md`.
- [x] Enforce blocking merge rule based on required gate labels/status checks in repository branch protection.
=======
...
=======
- [ ] Extend central `run-tasks-agent` from read-only planning to execute orchestration mode (create parent/subtasks, run worker-agent loops, then docs agent).
>>>>>>> 9eca956 (add  agents workflow)
...
- [x] Refactor module CI template to `module-only required + cms-integration optional` in `.github/workflows/module-ci.yml.template`.
- [x] Add frontend CI template with required quality/build/contract checks in `.github/workflows/frontend-ci.yml.template`.
- [x] Add CI rollout/install automation scripts for external module/frontend repos in `scripts/workflow/install-module-ci-template.sh` and `scripts/workflow/install-frontend-ci-template.sh`.
- [x] Add module/frontend branch-protection automation script in `scripts/workflow/apply-module-frontend-branch-protection.sh` with rollout instructions in `docs/workflow/rollout/module-frontend-ci-rollout.md`.
